...
export const GenRoute = <T extends TMaterials | TComponents | TNpc>(model: Model<T>): Router => {
    ...
    route.get('/all', async (req, res: TRes) => {
        const queryNpc = await model.find();
    ...
    route.get('/one/:id', validateMW, async (req: TGetOneReq, res: TRes) => {
        ...
        const queryNpc = await model.findById(getObjId(id));
    ...
    route.put('/one/:id', validateMW, async (req: TPostReq, res: TRes) => {
        const id = req.params.id;
        const data = await model.findById(getObjId(id));
    ...
    route.delete('/one/:id', validateMW, async (req: TGetOneReq, res: TRes) => {
        const id = req.params.id;
        const queryData = await model.findByIdAndDelete(getObjId(id));
